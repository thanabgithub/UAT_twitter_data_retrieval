...
def get_woeid_trends():
    available_data_to_woeid = pd.read_excel(
        "app/data/ELT_JP_WOEID.xlsx", index_col="Area"
    ).astype("string")
...
def finalize_woeid_trends_sql_format(
    as_of: list, wip_woeid_trends_df: pd.DataFrame()
) -> pd.DataFrame():
    ...
    fin_woeid_trends_df = pd.concat(
        [temp_datetime, wip_woeid_trends_df.T.reset_index()], axis=1, ignore_index=True
    )
    ...
    fin_woeid_trends_df.ExtractDatetime = pd.to_datetime(
        fin_woeid_trends_df.ExtractDatetime
    )
...
def get_timeseries_trends(national_trends):
    ...
    for keyword in national_trends:
        ...
        df_kw = pd.concat(
            [pd.DataFrame(columns=["keyword"], data=[keyword] * len(df)), df],
            axis=1,
            ignore_index=True,
        )
...
def finalize_timeseries_trend_sql_format(wip_timeseries_trend_df):
    sq_lite_timeformatted_now: str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    sq_lite_timeformatted_now_lst = [sq_lite_timeformatted_now] * len(
        wip_timeseries_trend_df
    )
    temp = pd.DataFrame(sq_lite_timeformatted_now_lst)
    fin_timeseries_trend_df = pd.concat(
        [
            pd.DataFrame(sq_lite_timeformatted_now_lst),
            wip_timeseries_trend_df.reset_index(drop=True),
        ],
        axis=1,
        ignore_index=True,
    )

    fin_timeseries_trend_df.columns = [
        "ExtractDatetime",
        "Keyword",
        "HistoricalDatetime",
        "Counts",
    ...
    fin_timeseries_trend_df.ExtractDatetime = pd.to_datetime(
        fin_timeseries_trend_df.ExtractDatetime
    )
    fin_timeseries_trend_df.HistoricalDatetime = pd.to_datetime(
        fin_timeseries_trend_df.HistoricalDatetime
    )
...
def insert_timeseries_trends(engine, fin_timeseries_trend_df):
    ...
    fin_timeseries_trend_df.to_sql(
        name="sqltable_temp",
        con=engine,
        if_exists="replace",
        index=False,
